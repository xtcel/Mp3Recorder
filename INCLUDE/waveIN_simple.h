		...
		switch (msg.message) {
			...
			case MM_WIM_DATA:
				...
				if ((((WAVEHDR *)msg.lParam)->dwBytesRecorded) && (_this->m_Receiver)) {
					...
					DWORD bytrecd = ((WAVEHDR*)msg.lParam)->dwBytesRecorded;
					LPSTR pcmData = ((WAVEHDR*)msg.lParam)->lpData;
					_this->getPcmDB((unsigned char *)pcmData, bytrecd);
				}
...
int CWaveINSimple::getPcmDB(const unsigned char* pcmdata, size_t size) {
